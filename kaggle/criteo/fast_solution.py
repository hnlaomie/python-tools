...
loss = 0.
for t, row in enumerate(DictReader(open(train), delimiter=',', lineterminator='\n')):
...
with open('/home/laomie/submission1234.csv', 'w') as submission:
    submission.write('LineNumber,Predicted\n')
    for t, row in enumerate(DictReader(open(test), delimiter=',', lineterminator='\n')):
        #Id = row['Id']
        #del row['Id']
        x = get_x(row, D)
        p = get_p(x, w)
